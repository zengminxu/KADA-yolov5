...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Backward
            torch.use_deterministic_algorithms(False) # wind -- EMA 注意力机制才加上的，平时要去掉
    ...
    optimizer = nni.trace(optimizer)(model.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/mobilenetv3-MLCA.yaml', help='model.yaml path')
