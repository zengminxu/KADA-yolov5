...
def training_model(model: torch.nn.Module, optimizer: Optimizer, training_step: Callable, scheduler: Union[SCHEDULER, None] = None,
    ...
    model.train()
    max_epochs = max_epochs if max_epochs else 1 if max_steps is None else 1
    current_steps = 0
...
# start = time.time()
# training_model(model, optimizer, training_step, None, None, 1)
# print(f'pure training 1 epochs: {time.time() - start}s')
# start = time.time()
